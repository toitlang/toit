...
// only call PLAT functions that are LINKED INTO the base image. This explicit
// allowlist preserves the reviewed driver, low-power, platform-service,
// networking, RTOS, libc/libm, and limited runtime surface available before
// slots were separated from the base. It is intentionally broader than the
// current slot: removing an entry saves little but can force deployed devices
// to take a full base reflash instead of an OTA.
//
// Do not generate this list from every symbol in the SDK. Additions must have
// a foreseeable slot use and are base-contract changes; obsolete entries may
// be removed only when publishing a new base version. Low-level entries here
// are native link capabilities, not public Toit APIs. Toit-facing mutation
// primitives still need their own privilege checks.
//
...
// Preserve only the limited compiler/C++ helpers already in the reviewed
// base surface. This is not a general libstdc++ ABI: helpers absent here are
// linked into the slot, and adding more requires the same base-contract
// review as any PLAT symbol.
