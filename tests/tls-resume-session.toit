...
test-site host/string --read-data/bool=true -> none:
  ...
  3.repeat: | iteration |
    ...
    socket := tls.Socket.client raw
      ...
      --root-certificates=[BALTIMORE-CYBERTRUST-ROOT, GLOBALSIGN-ROOT-CA, DIGICERT-GLOBAL-ROOT-G2, DIGICERT-GLOBAL-ROOT-CA, ISRG-ROOT-X1]
      --server-name=host
    ...
    print "Handshake complete ($(%22s method), suite $(%4x suite)) to $(%16s host) in $(%4d duration.in-ms) ms"
...
ISRG-ROOT-X1 ::= net.Certificate.parse ISRG-ROOT-X1-TEXT_
...
*/
DIGICERT-GLOBAL-ROOT-CA ::= net.Certificate.parse DIGICERT-GLOBAL-ROOT-CA-BYTES_
