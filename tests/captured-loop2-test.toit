...
main:
  ...
  for x := 0;
      ...
      x++:
    true
  expect-equals (ITERATIONS + 1) funs.size
  funs.size.repeat:
  ...
  for x := 0; x < ITERATIONS; (run:
      ...
      x++):
    true
  expect-equals ITERATIONS funs.size
  funs.size.repeat:
    expect-equals (it + 1) funs[it].call
  ...
  for i := 0; i < ITERATIONS - 1; i++:
    ...
    expect-equals (i + 2) funs[i].call
  expect-equals ITERATIONS funs[ITERATIONS - 1].call
