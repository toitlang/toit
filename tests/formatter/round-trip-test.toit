...
// Mirrors source trees into a tmp directory (preserving structure so relative
...
main args:
  ...
  with-tmp-dir: | tmp-dir/string |
    count := 0
    ["lib", "tools", "examples"].do: | root/string |
      source-root := "$repo-root/$root"
      if file.is-directory source-root:
        mirror-tree source-root "$tmp-dir/$root"
        walk-toit-files "$tmp-dir/$root": | src-path/string |
