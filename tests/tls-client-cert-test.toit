...
main:
  test-site "client.badssl.com" null null false null
...
test-site url cert key expect-ok expected-cert-name:
  host := url
  port := 443
  if (url.index-of ":") != -1:
    array := url.split ":"
  ...
  load-limiter.inc
  if expect-ok:
    task:: working-site host port cert key expected-cert-name
  else:
    task:: non-working-site host port cert key expected-cert-name

non-working-site site port cert key expected-cert-name:
  exception ::= catch:
    connect-to-site site port cert key expected-cert-name
    load-limiter.log-test-failure "*** Incorrectly failed to reject SSL connection to $site ***"
  load-limiter.dec

working-site host port cert key expected-cert-name:
  error := true
  try:
    ...
    if error:
      load-limiter.log-test-failure "*** Incorrectly failed to connected to $host ***"
    load-limiter.dec
