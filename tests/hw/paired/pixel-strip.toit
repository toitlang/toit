...
// Keep full waveform captures within the classic ESP32 tester memory.
// Long UART bursts are checked separately with filtered gap detection.
PIXELS ::= 7
...
run session/Session ready-pin/int --backends/List=["rmt", "uart"]:
  backends.do: | backend |
    session.run-case "Pixel strip $backend waveform":
      uart-tests.window session ready-pin
          --transmit=: | ready |
            strip := backend == "rmt"
                ? (PixelStrip.rmt PIXELS --pin=session.tx)
                : (PixelStrip.uart PIXELS --pin=session.tx)
            try:
              red := ByteArray PIXELS
              green := ByteArray PIXELS
              blue := ByteArray PIXELS
              uart-tests.await-trigger ready
              fill-frame 3 red green blue
              strip.output red green blue
              // UART output is buffered; keep its port alive until observed.
              ready.wait-for 0
            finally:
              strip.close
          --observe=: | ready |
            // Leave room for leading symbols so the verifier can diagnose
            // incorrect timing/data instead of just a truncated capture.
            input := rmt.In session.rx --resolution=RESOLUTION --memory-blocks=(RMT-MEMORY-BLOCKS + 1)
            try:
              input.start-reading --max-ns=50_000
              ready.set 1
              signals := input.wait-for-data
              print "Pixel $backend captured $signals.size signals"
              validate-capture signals backend 3
            finally:
              input.close
