...
Lane "uart2" — the mini-jag rescue lane (same wiring and TCP port as uart-bridge-esp32.toit): ESP32 IO27 <- EC618 UART2 TX, IO14 -> EC618 UART2 RX, TCP 18555. Lane "uart1": ESP32 IO4 <- EC618 PAD34 (UART1 TX), ESP32 IO16 -> EC618 PAD33 (UART1 RX), TCP 18556.

Keeping both lanes live at the same time is the point: while an experiment talks to the EC618 over the UART1 lane (e.g. reproducing the quirky shared-console RX deafness after an `ec618.set-console-uart 1` flip), the UART2 rescue lane stays reachable as the way back.
...
(--fast-baud 115200 disables the baud hop: the bridge UARTs are fixed.) One client per lane at a time; the next is accepted after a disconnect.
