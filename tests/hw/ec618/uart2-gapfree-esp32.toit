...
Watches the EC618's UART2 TX wire (IO27) with the pulse counter behind its maximum-ish glitch filter (12 us). The EC618 sends all-0x00 bytes (1 stop bit), so a gap-free stream never holds the line high longer than ONE bit time — the whole stream is rejected by the filter as jitter and the filtered signal stays low. Only a PAUSE (line idles high) or the end of the stream survives the filter as a rising edge:

rising-edge count = pauses + 1 (the trailing idle).

Detection floor = the filter: pauses shorter than ~12 us pass unseen (PCNT's filter caps at ~12.7 us; an RMT idle-threshold variant could reach ~3 us if ever needed). LED-strip latch thresholds are >=50 us (rare clones ~9 us), and software/DMA seams are tens of us, so 12 us covers the failure mode that matters. 115200 is the baud floor: its 8.7 us stop bit must stay below the filter.
