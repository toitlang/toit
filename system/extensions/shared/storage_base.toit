...
abstract class StorageServiceProviderBase extends ServiceProvider
  ...
  handle pid/int client/int index/int arguments/any -> any:
    if index == StorageService.GET_INDEX:
      ...
      return open_bucket client arguments
    unreachable

  abstract open_bucket client/int name/string -> BucketResource
...
class FlashKeyValueBucketResource extends BucketResource:
  static group_ ::= flash_kv_init_ "nvs" "storage" false
  name/string
  keys ::= {:}
  ...
  get key/string -> ByteArray?:
    return flash_kv_read_bytes_ group_ (flash_key_ key)

  set key/string value/ByteArray -> none:
    flash_kv_write_bytes_ group_ (flash_key_ key) value

  remove key/string -> none:
    flash_kv_delete_ group_ (flash_key_ key)

  flash_key_ key/string -> string:
    return keys.get key --init=: (uuid.uuid5 name key).stringify[..13]
