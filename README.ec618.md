...
- Two-stage frozen-base link: slots link directly against the selected
  `base.elf`, and each slot carries the exact base ID it requires.
- Dual-slot OTA with trial boot, rollback, and SRL3 relocate-on-write.

## Position independence (dual-slot OTA)

The slot linker resolves VM→PLAT calls directly from the selected `base.elf`
through `--just-symbols`. In-slot calls may be inlined normally. Non-inlined
calls to the base are ordinary Thumb branches; SRL3 records every branch that
escapes the slot and adjusts its immediate when the slot moves. The base ID in
SRL3 prevents an image from running with different symbol addresses.

### Slot relocation table

The VM's canonical image is linked at a neutral base. The words that must change
when it is placed in slot A or slot B are captured in a relocation table by
...
- **ABS32 pointers into the slot** (vtables, const pointer tables,
  ...
  moving content.
- **Branches that escape the slot** (direct VM→PLAT calls): the device
  subtracts `delta` from the Thumb BL immediate.

Everything else is already slot-independent. The table is delta-encoded and
...
__vm_a_start ─┐
  ...
  [ free ]
  [ SRL3 table ]         relocation sites + required base ID
...
the extension there and **extends** the SRL3 table with the extension's absolute
