...
IMAGE_DATA_SIZE ::= 100 * 1024
IMAGE_DATA_OFFSET ::= 296

IMAGE_DATA_MAGIC_1 := 0x7017da7a
IMAGE_DATA_MAGIC_2 := 0x7017ed

/**
  usage: inject_image <image> <bin>
*/
main args/List:
  parser := ArgumentParser
  parsed := parser.parse args
  image/string := parsed.rest[0] as string
  bin/string := parsed.rest[1] as string

  image_data := file.read_content image
  bin_data := file.read_content bin

  result := inject_image image_data bin_data

  bin_stream := file.Stream.for_write bin
...
inject_image image_data/ByteArray bin_data/ByteArray -> ByteArray:
  ...
  if image_data.size > image_data_size:
    throw "iamge too big to inline into binary"

  bin_data.replace image_data_offset (ByteArray image_data_size)  // Zero out area.
  bin_data.replace image_data_offset image_data
...
get_image_data_position bytes/ByteArray -> List:
  ...
  for i := 0; i < bytes.size; i += WORD_SIZE:
    ...
    if word_1 == IMAGE_DATA_MAGIC_1:
      ...
      for j := 0x200 + WORD_SIZE; j <= 0x100000 + WORD_SIZE and i + j < bytes.size; j += 0x200:
