...
class Port extends Object with io.InMixin implements reader.Reader:
  ...
  # EC618 (Air780E/Air780EG, Air600E, ...)

  On the EC618, $gpio.Pin numbers are physical PAD indices (1..47). For
    most cases you should use the helpers in the `ec618` library rather
    than constructing $gpio.Pin directly: `Ec618.uart0`, `Ec618.uart1`,
    `Ec618.uart2` produce a fully-configured $Port; `Ec618.gpio N` and
    `Ec618.pad N` produce a $gpio.Pin for the relevant PAD.

  ## UART pin layouts

  Each controller has one or more pin "mappings". The PAD column is the
    physical chip pin; the GPIO column is what most module silkscreens
    label the pad as.

  | UART | Mapping | TX (PAD/GPIO) | RX (PAD/GPIO) | RTS    | CTS    |
  |------|---------|---------------|---------------|--------|--------|
  | 0    | 0       | 30 / GPIO15   | 29 / GPIO14   | PAD27 / GPIO12 | PAD28 / GPIO13 |
  | 0    | 1       | 32 / GPIO17   | 31 / GPIO16   | (none) | (none) |
  | 1    | 0       | 34 / GPIO19   | 33 / GPIO18   | PAD31 / GPIO16 | PAD32 / GPIO17 |
  | 1    | 1 (CTS) | (n/a)         | (n/a)         | (n/a)  | PAD22 / GPIO11 alt |
  | 2    | 0       | 26 / GPIO11   | 25 / GPIO10   | (none) | (none) |
  | 2    | 1       | 28 / GPIO13   | 27 / GPIO12   | (none) | (none) |

  ## Notes

  - UART0 carries the bootloader banner (BROM at chip reset) and the PLAT
  ...
  - UART1 is the only UART that can wake the chip from deep sleep. On
    ...
    byte is lost.
  - UART1 has modem-control lines on fixed pads: DTR=GPIO11, RI=GPIO24,
    DCD=GPIO25. These are exposed as ordinary GPIOs; Toit does not drive
    them automatically.
  - $invert-tx, $invert-rx, $MODE-IRDA and $high-priority are not
    supported on the EC618. $MODE-RS485-HALF-DUPLEX is supported and
    uses $rts as the direction pin (any GPIO-capable PAD works).

  ## Luatos Air modules

  Air* modules are assembled around the EC618 by Luatos/AirM2M. Most
    modules pin out only a subset of the GPIOs, and some remap UART2 at
    the factory:

  - Air780E / Air780E-L: UART0 on GPIO14/15 (mapping 0), UART1 on
    GPIO18/19, UART2 on GPIO10/11 (mapping 0).
  - Air780EG / Air780EUG: the GNSS co-processor occupies GPIO10/11, so
    UART2 should be opened with `--mapping=1` (TX=GPIO13, RX=GPIO12).
    The default mapping silently talks to the GNSS rails on these
    modules.
  - Air700E (small QFN): UART0 and UART1 are exposed; UART2 is not
    broken out to a package pin.
  - Air600E: same layout as Air780E.

  Note that some dev-board silkscreens label both PADs of a multi-pad
    GPIO as "GPIO11" (or "TX2"), even though only one of those PADs can
    actually carry the UART2 TX function. The other PAD can carry
    different functions (for GPIO11 these are UART1 CTS and SPI0 MOSI).
    Trust the iomux table over the silkscreen.
  */
  constructor
