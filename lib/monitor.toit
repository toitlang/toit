...
/**
Task cache that that caches a couple of tasks to reduce the amount of ned
  tasks created to handle small tasks.

If the cache runs out of tasks, it will create a new task to execute the new
  lambda.
*/
monitor TaskCache_:
  static MAX_IDLE_TASKS/int ::= 2
  queue_/List := []
  tasks_idle_/int := 0

  next -> Lambda?:
    if tasks_idle_ >= MAX_IDLE_TASKS: return null
    tasks_idle_++
    await: not queue_.is_empty
    tasks_idle_--
    lambda := queue_.first
    // TODO(anders): Use a real queue.
    if queue_.size == 1:
      queue_.resize 0
    else:
      queue_ = queue_.copy 1
    return lambda

  /**
  Run the $lambda in a task.

  If no tasks are available, then start a new one.
  */
  run lambda/Lambda:
    queue_.add lambda
    // If we have more items to run than tasks available, start a new one.
    // Note that worst-case behavior is one new task per lambda.
    if queue_.size > tasks_idle_: task:: broker_task_ this

  static broker_task_ queue/TaskCache_:
    while lambda := queue.next:
      lambda.call
