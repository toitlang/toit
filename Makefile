...
ec618: check-env host-tools
	...
	# "Wrap only the VM" (Option M): rewrite the VM archives' PLAT references
	# <sym> -> __wrap_<sym> so VM calls reach the in-slot jump-table stubs, while
	# PLAT/RAM-resident code keeps calling the real symbols directly. PLAT must
	# never branch into an in-slot stub: that faults if its slot is the one being
	# erased during a B->A OTA (the trampolines live in the slot). The map is
	# generated by tools/ec618/gen-plat-jt.toit alongside plat_jt.c; the rewrite
	# is idempotent (a second pass finds no un-rewritten <sym>).
	for a in $(BUILD)/ec618/src/libtoit_vm.a \
	         $(BUILD)/ec618/mbedtls/library/libmbedtls.a \
	         $(BUILD)/ec618/mbedtls/library/libmbedx509.a \
	         $(BUILD)/ec618/mbedtls/library/libmbedcrypto.a; do \
		$(EC618_GCC_PATH)/bin/arm-none-eabi-objcopy --redefine-syms=$(CURDIR)/tools/ec618/plat_jt_redefine.txt $$a; \
	done
