...
// GPIO port/pin decomposition: port = pin / 16, pin_in_port = pin % 16.
...
PRIMITIVE(use) {
  ARGS(GpioResourceGroup, group, int, pin, bool, output);
  ByteArray* proxy = process->object_heap()->allocate_proxy();
  if (proxy == null) FAIL(ALLOCATION_FAILED);

  if (pin < 0 || pin > 31) FAIL(OUT_OF_RANGE);
  ...
  config.pinDirection = output ? GPIO_DIRECTION_OUTPUT : GPIO_DIRECTION_INPUT;
  config.misc.initOutput = 0;
  GPIO_pinConfig(to_port(pin), to_pin_mask(pin), &config);
...
PRIMITIVE(unuse) {
  ...
  // Disable interrupts on release.
  int pin = resource->pin();
  GPIO_interruptConfig(to_port(pin), to_pin_mask(pin), GPIO_INTERRUPT_DISABLED);
...
PRIMITIVE(config) {
  ARGS(GpioResource, resource, bool, output, bool, open_drain,
       bool, pull_up, bool, pull_down, int, value, int, frequency);
  USE(open_drain); USE(pull_up); USE(pull_down); USE(frequency);

  int pin = resource->pin();
  GpioPinConfig_t config;
  memset(&config, 0, sizeof(config));
  config.pinDirection = output ? GPIO_DIRECTION_OUTPUT : GPIO_DIRECTION_INPUT;
  config.misc.initOutput = value;
  GPIO_pinConfig(to_port(pin), to_pin_mask(pin), &config);
...
PRIMITIVE(get) {
  ...
  int pin = resource->pin();
  return Smi::from(GPIO_pinRead(to_port(pin), to_pin_mask(pin)) ? 1 : 0);
}

PRIMITIVE(set) {
  ARGS(GpioResource, resource, int, value);
  int pin = resource->pin();
  uint16_t mask = to_pin_mask(pin);
  GPIO_pinWrite(to_port(pin), mask, value ? mask : 0);
  return process->null_object();
}
...
PRIMITIVE(config_interrupt) {
  ...
  if (enable) {
    ...
    GPIO_interruptConfig(to_port(pin), to_pin_mask(pin), int_config);
  } else {
    GPIO_interruptConfig(to_port(pin), to_pin_mask(pin), GPIO_INTERRUPT_DISABLED);
  }
