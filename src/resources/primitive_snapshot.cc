...
PRIMITIVE(launch) {
  ...
  if (!initial_chunk) ALLOCATION_FAILED;
  ...
  int pid = pass_args
     ? VM::current()->scheduler()->run_program(program, process->args(), process_group, initial_chunk)
     : VM::current()->scheduler()->run_program(program, {}, process_group, initial_chunk);
