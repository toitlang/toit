...
static bool equiv_toitdoc_reference(ToitdocReference* a,
                                    ToitdocReference* b) {
...
// Per-comparison state. The pointer is thread-local so independent compiler
// jobs cannot race; saving and restoring it in ast_equivalent also makes a
// nested comparison safe.
struct MismatchState {
  Node* a = null;
  Node* b = null;
  bool recorded = false;
};
thread_local MismatchState* mismatch_state_ = null;

static void record_mismatch(Node* a, Node* b) {
  ASSERT(mismatch_state_ != null);
  if (mismatch_state_->recorded) return;
  mismatch_state_->recorded = true;
  mismatch_state_->a = a;
  mismatch_state_->b = b;
}

static bool equiv_no_record(Node* a, Node* b);
...
static bool equiv_no_record(Node* a, Node* b) {
  ...
  // formatter input, and future AST kinds must be added explicitly instead
  // of silently weakening the verifier.
  return false;
}
...
bool ast_equivalent(Unit* a, Unit* b, Node** mismatch_a, Node** mismatch_b) {
  MismatchState state;
  MismatchState* previous_state = mismatch_state_;
  mismatch_state_ = &state;
  bool result = equiv(a, b);
  mismatch_state_ = previous_state;
  if (mismatch_a != null) *mismatch_a = state.a;
  if (mismatch_b != null) *mismatch_b = state.b;
  return result;
}
