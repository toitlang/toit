...
// The deepest node pair at which a mismatch was detected; recorded on
// the way out of the recursion (the first failure to be recorded is the
// deepest one).
static Node* mismatch_a_ = null;
static Node* mismatch_b_ = null;
static bool mismatch_recorded_ = false;
...
bool ast_equivalent(Unit* a, Unit* b, Node** mismatch_a, Node** mismatch_b) {
  mismatch_recorded_ = false;
  mismatch_a_ = null;
  mismatch_b_ = null;
  bool result = equiv(a, b);
  if (mismatch_a != null) *mismatch_a = mismatch_a_;
  if (mismatch_b != null) *mismatch_b = mismatch_b_;
  return result;
}
