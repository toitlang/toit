...
class Lowering {
 public:
  ...
  std::string run(bool* all_trivia_emitted) {
    ...
    std::string result = print_doc(doc, 0, style_, &emitted_comments);
 ...
 private:
  ...
  Doc* expr(Expression* e, int outer) {
    ...
    while (e != null && e->is_Parenthesis()) {
      ...
      if (is_trivial_inner(peel_parens(inner))) {
        ...
        // A suite ends at the line boundary. If an enclosing construct
        // appended more tokens on this line, the parser would absorb the
        // closing parenthesis and those tokens into the suite body.
        if (contains_suite(inner)) docs.push_back(b_.break_parent());
        return b_.concat(std::move(docs));
      }
    }
    if (e->is_Block() || e->is_Lambda()) {
  ...
  Doc* ternary(If* if_node, int outer) {
    ...
    Doc* result = b_.group(b_.concat(std::move(docs)));
  ...
  Doc* unary(Unary* u, int outer) {
    ...
    std::vector<Doc*> docs;
    if (parens) docs.push_back(b_.unmeasured_text("("));
  ...
  Doc* call_argument_flat(Expression* argument) {
    if (argument->is_NamedArgument()) {
  ...
  Doc* call_argument(Expression* argument) {
    if (argument->is_NamedArgument()) {
  ...
  Doc* call(Call* c, int outer) {
    ...
    while (glued_args < head_args
      ...
      Doc* doc = call_argument_flat(arguments[glued_args]);
    ...
    if (hugged != null) {
      ...
      while (i < head_args) {
        ...
        if (named_suite_in_head && i == arguments.length() - 1) {
          ...
          argument_docs.push_back(call_argument(argument));
        }
      ...
      for (int i = first_blockish; i < arguments.length(); i++) {
        ...
        if (trivia != null) {
          for (auto& comment : trivia->leading) {
            ...
            line_contents.push_back(comment_doc(comment));
            line_contents.push_back(b_.hardline());
          }
        }
        line_contents.push_back(line_doc);
  ...
  Doc* collection(List<Expression*> elements, const char* open, const char* close, Expression* owner) {
    ...
    for (auto element : elements) {
      ...
      Doc* element_doc = expr(element, PRECEDENCE_NONE);
      int width = DocBuilder::measure_flat(element_doc);
      if (!DocBuilder::has_finite_flat(element_doc)) can_pack = false;
      element_docs.push_back(element_doc);
    ...
    if (can_pack) {
      ...
      for (int i = 0; i < count; i++) {
        ...
        flat.push_back(element_docs[i]);
      }

      std::vector<Doc*> packed;
      int next_element = 0;
      ...
      for (int row = 0; row < row_count; row++) {
        ...
        std::vector<Doc*> row_docs;
        for (int i = row_start; i < next_element; i++) {
          if (i > row_start) row_docs.push_back(b_.line());
          row_docs.push_back(element_docs[i]);
          row_docs.push_back(b_.text(","));
        }
        packed.push_back(b_.group(b_.concat(std::move(row_docs))));
        remaining_width -= row_width;
      }

      Doc* contents = b_.if_broken(
          b_.indent(style_.indent_step,
                    b_.concat({b_.hardline(),
                               b_.concat(std::move(packed))})),
          b_.concat(std::move(flat)));
      Doc* closing = b_.if_broken(
    ...
    for (int i = 0; i < elements.length(); i++) {
      ...
      append_element(&inner, elements[i], element_docs[i], last);
    }
  ...
  Doc* collection_after_open(Expression* owner) {
    const NodeTrivia* trivia = trivia_.find(owner);
  ...
  Doc* method_declaration(Method* method) {
    ...
    if (body->expressions().is_empty()) {
      ...
      docs.push_back(trailing_trivia(body));
  ...
  Doc* unit_doc() {
    ...
    if (!trivia_.preamble().empty()) {
      docs.push_back(b_.text(trivia_.preamble()));
      first_entity = false;
    }
