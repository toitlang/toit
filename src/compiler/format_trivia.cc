...
class TriviaWalker {
 ...
 private:
  ...
  // TODO(florian): Use indentation to route own-line comments for every
  // suite, not only comment-only method bodies. A comment indented inside a
  // preceding control-flow suite must remain dangling trivia of that suite,
  // including when the suite contains no AST statements:
  //
  //   if not x:
  //     // Do nothing.
  //
  // For now, only a method whose body consists entirely of comments
  // (`on-foo:` followed by indented comment lines) owns deeper comments;
  // otherwise they become leading trivia of the next sibling.
