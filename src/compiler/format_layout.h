...
// Tunable layout preferences. Grammar-preserving choices stay in the AST
// lowering; this structure only controls widths, indentation, and the cost of
// choosing a broken alternative.
struct FormatStyle {
...
// A node in the formatter's layout tree. AST lowering builds the tree and
// describes its legal flat and broken forms. LayoutPrinter later selects the
// form of each GROUP and renders the selected tree.
//
// Nodes are allocated through LayoutBuilder and live as long as it does.
class Layout {
public:
  enum Kind {
  ...
  const std::vector<Layout *> &children() const { return children_; }
...
private:
  ...
  Kind kind_;
  // Only the kinds named on each line read the corresponding field. Keeping
  ...
  // IDs recorded only if this particular Layout is actually printed. Used by
  // callers to prove lossless trivia emission across layout alternatives.
  std::vector<int> emission_ids_;

  // Memoized approximate single-line width, counted as UTF-8 code points;
...
// Allocates and owns Layout nodes.
class LayoutBuilder {
public:
  LayoutBuilder() {}
  ~LayoutBuilder();
  ...
  Layout *unmeasured_text(std::string s);
  // `original_column >= 0` requests delta-shifting of interior lines
  // (see Layout::VERBATIM).
  Layout *verbatim(std::string s, int original_column = -1);
  Layout *concat(std::vector<Layout *> children);
  Layout *group(Layout *child, int budget = -1, int alternative_lines = 0);
  Layout *indent(int delta, Layout *child);
  Layout *line();     // Newline or single space.
  Layout *softline(); // Newline or nothing.
  Layout *hardline();
  Layout *break_parent();
  ...
  static int measure_flat(const Layout *layout);
  static bool has_finite_flat(const Layout *layout);

private:
  Layout *make(Layout::Kind kind);
