...
Doc* DocBuilder::unmeasured_text(std::string s) {
  Doc* doc = text(std::move(s));
  doc->flat_width_ = 0;
  return doc;
}
...
Doc* DocBuilder::group(Doc* child, int budget, int alternative_lines) {
  Doc* doc = make(Doc::GROUP);
  doc->children_.push_back(child);
  doc->budget_ = budget;
  doc->alternative_lines_ = alternative_lines;
  return doc;
}
...
Doc* DocBuilder::track(Doc* doc, std::vector<int> emission_ids) {
  doc->emission_ids_ = std::move(emission_ids);
  return doc;
}
...
int DocBuilder::measure_flat(const Doc* doc) {
  ...
  switch (doc->kind()) {
      ...
      case Doc::BREAK_PARENT:
        ...
        break;
      case Doc::IF_BROKEN:
        width = measure_flat(doc->flat_alternative());
        break;
  }
...
bool DocBuilder::has_finite_flat(const Doc* doc) {
  return measure_flat(doc) < INFINITE_WIDTH;
}

class DocPrinter {
 public:
  DocPrinter(const FormatStyle& style,
             int base_indent,
  ...
  void print(const Doc* doc, int indent, bool flat) {
    if (emitted_ids_ != null) {
      for (int id : doc->emission_ids()) emitted_ids_->push_back(id);
    }
    switch (doc->kind()) {
      case Doc::TEXT:
        emit_text(doc->text());
 ...
 private:
  ...
  // Whether `doc` fits as a flat group. Width is node-relative. The
  // current indentation adds gentle pressure, but there is no absolute
  // maximum line width.
  bool fits(const Doc* doc) {
    int width = DocBuilder::measure_flat(doc);
    ...
    int budget = doc->budget() >= 0 ? doc->budget() : style_.preferred_extent;
    ...
    int costly_lines =
        doc->alternative_lines() - style_.alternative_line_threshold;
    if (costly_lines < 0) costly_lines = 0;
    int alternative_allowance =
        costly_lines * style_.alternative_line_extent;
...
std::string print_doc(const Doc* doc,
                      ...
                      std::vector<int>* emitted_ids) {
  DocPrinter printer(style, base_indent, emitted_ids);
