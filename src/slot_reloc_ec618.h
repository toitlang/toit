...
// A parsed, zero-copy view over an "SRL3" reloc-table blob. The blob stays
...
struct SlotRelocTable {
  ...
  uint32_t base_version;         // base-vN the slot was linked against.
  const uint8_t* base_fp;        // 16-byte base fingerprint (into the blob).
...
// there as `[ SRL3 table (N bytes) ][ N : uint32 little-endian ]`, with N the
...
// Serializes `table_blob` (an "SRL3" blob of `len` bytes) plus its trailing
...
//
//   [ table_size : u32 ][ SRL3 table ][ VM body + extension ][ VM .data init ]
//
// while the physical slot stores it tail-first:
//
//   [ VM body + ext ][ VM .data init ][ free ][ SRL3 table ][ table_size : word ].
//
