// Copyright (C) 2026 Toit contributors.
// Use of this source code is governed by a Zero-Clause BSD license that can
// be found in the tests/LICENSE file.

import net
import net.modules.tcp
import system

TRANSFER-SIZES ::= [
  173_025,
  245_688,
  1_212_346,
  8,
  63,
  1_390_176,
  168_486,
  45,
  23_417_108,
]

REQUEST-HEADER-SIZE ::= 256

TRANSFER-DELAYS-MS ::= [
  0,
  150,
  675,
  126,
  1,
  0,
  1,
  1,
  0,
]

main args:
  if args.size != 2 and args.size != 3: throw "EXPECTED_MODE_PORT_AND_OPTIONAL_PROFILE"
  port := int.parse args[1]
  profile := "stream"
  if args.size == 3: profile = args[2]
  if profile != "stream" and profile != "http-boundaries": throw "UNKNOWN_PROFILE"
  http-boundaries := profile == "http-boundaries"
  network := net.open
  if args[0] == "server":
    run-server network port http-boundaries profile
  else if args[0] == "client":
    run-client network port http-boundaries
  else:
    throw "UNKNOWN_MODE"

run-server network/net.Client port/int http-boundaries/bool profile/string:
  server := tcp.TcpServerSocket network
  server.listen "127.0.0.1" port
  print "TCP_CLAMP_REPRO ready port=$port profile=$profile"

  socket := server.accept
  retained := [ByteArray 52_000_000]
  TRANSFER-SIZES.size.repeat: | index |
    if http-boundaries: read-exactly socket REQUEST-HEADER-SIZE
    started := Time.monotonic-us
    body := read-exactly socket TRANSFER-SIZES[index]
    retained.add body
    elapsed := Time.monotonic-us - started
    print "TCP_CLAMP_REPRO transfer=$index size=$body.size elapsed_us=$elapsed window_size=$(socket.window-size)"
    if http-boundaries:
      socket.no-delay = false
      socket.out.write (ByteArray 32)
      socket.out.write (ByteArray 10)
      socket.no-delay = true
    else:
      socket.out.write (ByteArray 42)

  socket.close
  server.close

run-client network/net.Client port/int http-boundaries/bool:
  socket := tcp.TcpSocket network
  socket.connect "127.0.0.1" port
  TRANSFER-SIZES.size.repeat: | index |
    delay := TRANSFER-DELAYS-MS[index]
    if delay: sleep --ms=delay
    if http-boundaries:
      socket.no-delay = false
      socket.out.write (ByteArray REQUEST-HEADER-SIZE)
      socket.no-delay = true
    socket.out.write (ByteArray TRANSFER-SIZES[index])
    read-exactly socket 42
  socket.out.close
  socket.close

read-exactly socket size/int -> ByteArray:
  chunks := []
  read-size := 0
  while read-size < size:
    chunk := socket.in.read --max-size=(size - read-size)
    if chunk == null: throw "UNEXPECTED_END_OF_STREAM"
    chunks.add chunk
    read-size += chunk.size

  // Match io.Reader.read-all: retain every primitive read buffer until the
  // body is complete, then copy them into one contiguous result.
  result := ByteArray size
  offset := 0
  chunks.do:
    result.replace offset it
    offset += it.size
  return result
