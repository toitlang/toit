...
public final class Richards {
	...
	public static void inst_main(String args[]) {
		int iterations = 10;
		String options = "";
		...
		long total_ms = endTime - startTime;
		System.out.println(
				"Richards\tJava\t" + options + "\t" + iterations + "x\t" + ((double) total_ms / iterations) + " ms");
	}
	...
	/**
	 * These two constants specify how many times a packet is queued and how many times a task is put on hold in a
	 * correct run of richards. They don't have any meaning a such but are characteristic of a correct run so if the
	 * actual queue or hold count is different from the expected there must be a bug in the implementation.
	 **/
...
/**
 * A scheduler can be used to schedule a set of tasks based on their relative priorities. Scheduling is done by
 * maintaining a list of task control blocks which holds tasks and the data queue they are processing.
 */
class Scheduler {
	...
	/**
	 * Block the currently executing task and return the next task control block to run. The blocked task will not be
	 * made runnable until it is explicitly released, even if new work is added to it.
	 */
	...
	/**
	 * Suspend the currently executing task and return the next task control block to run. If new work is added to the
	 * suspended task it will be made runnable.
	 */
	...
	/**
	 * Add the specified packet to the end of the worklist used by the task associated with the packet and make the task
	 * runnable if it is currently suspended.
	 */
...
class TaskControlBlock {
	...
	TaskControlBlock run() {
		...
		if (this.state == STATE_SUSPENDED_RUNNABLE) {
			...
			this.state = this.queue == null ? STATE_RUNNING : STATE_RUNNABLE;
		} else {
			packet = null;
		}
	...
	/**
	 * Adds a packet to the worklist of this block's task, marks this as runnable if necessary, and returns the next
	 * runnable object to run (the one with the highest priority).
	 */
...
class WorkerTask extends Task {
	...
	TaskControlBlock run(Packet packet) {
		...
		if (this.v1 == Richards.ID_HANDLER_A) {
...
/**
 * A simple package of data that is manipulated by the tasks. The exact layout of the payload data carried by a packet
 * is not importaint, and neither is the nature of the work performed on packets by the tasks. Besides carrying data,
