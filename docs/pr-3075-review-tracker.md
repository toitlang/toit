...
| ADC | **Open** | The two application channels use a globally locked ownership pool, may coexist with each other, and reject a second owner of the same AIO input. `SimpleResource` already registered the resource automatically; its EC618 destructor now deinitializes and releases the channel on explicit close or forced container teardown. Trim/configuration initialization is serialized, and the vendor driver serializes cross-channel conversions through its IRQ-safe request queue. EC618 conversion primitives now only start or harvest the per-resource request; while it is pending, the public `Adc` method sleeps/yields in Toit under a per-instance monitor instead of busy-polling inside a primitive. Remaining: hardware-run the ownership and scheduler-progress regressions. |
