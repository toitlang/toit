...
| PWM | **Open** | Each channel now owns its teardown and returns its timer through the locked global resource pool, including forced container teardown. Physical pads are still not independently leased by the PWM channel. A requested factor of 1.0 still emits a short low notch. |
