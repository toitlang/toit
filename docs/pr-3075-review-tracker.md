...
| ADC | **Open** | The two application channels use a globally locked ownership pool, may coexist with each other, and reject a second owner of the same AIO input. `SimpleResource` already registered the resource automatically; its EC618 destructor now deinitializes and releases the channel on explicit close or forced container teardown. Trim/configuration initialization is serialized. Remaining: serialize conversions across the shared ADC engine and replace primitive-side busy polling with an event-driven wait. |
