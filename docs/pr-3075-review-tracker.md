...
| GPIO/pad ownership | **Open** | Every `gpio.Pin` now reserves both its physical EC618 pad and its GPIO controller bit atomically under the VM global lock. This enforces system-wide pin exclusivity, rejects alternate-pad aliases of an already-owned controller bit, and releases both identities through the resource-group teardown hook on explicit close or container death. Hardware coverage checks the initial output level, duplicate/alias rejection, explicit and forced release/reacquisition, and invalid values. Remaining: make the direct mux transitions and temporary GPIO use inside I2C/SPI/UART participate in the same locked ownership model, and ref-count AON-pad power. |
