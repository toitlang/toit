...
| Hardware tests | **Open** | Many later tests are better, but timing, duplicate protocols, swallowed exceptions, bring-up files, and incomplete contention/cancellation cases remain across the suite. Test comments now state their assumptions and contracts locally instead of relying on numbered issues or temporary design documents. |
