...
| Historical comments/docs | **Audit** | Production and test comments no longer rely on historical phase numbers, issue numbers, or temporary document paths. Several design documents still claim VM-to-PLAT calls use `g_plat_jt`, and the generated `plat_jt.h` survives despite the jump table's removal in `8d7dfb01`; audit and remove this archaeology without deleting the unrelated libc time wrappers. |
