...
> - **Pointer fix-up is an explicit SRL3 relocation table** (`--emit-relocs`,
>   delta-encoded), *not* the "magic-prefix pointer rewriting" sketched below.
>   SRL3 also relocates direct VM→PLAT Thumb branches and identifies the exact
>   base against which the slot linked.
...
> - **Tooling is Toit**, not Python. The runtime jump table and all of its
>   generator/wrapper machinery were removed. Slots now link directly against
>   the selected `base.elf`; `plat_keep.c` retains the supported base symbols.
>
> The "Trial boot + automatic rollback" concept below remains accurate. The
> FOTA, splice, magic-prefix, and jump-table mechanics are historical only.
...
### Historical jump-table proposal (removed)

This proposal used a fixed-address PLAT jump table. It was later removed:
in-slot inlining remains normal, while non-inlined VM→PLAT calls link directly
against the selected frozen base and their escaping Thumb branches are carried
in SRL3. The base id prevents a slot from being installed on a different base.
